...
namespace FerruzFlixMAUI.Services
{
    public partial class TmdbService
    {
        private const string ApiKey = "f795ce1d7b39be74c6aef150d1102657"; // generate it from tmdb website
        public const string TmdbHttpClientName = "Ferruzxca";
        ...
        public async Task<IEnumerable<Media>> SearchMoviesAsync(string query)
        {
            var response = await HttpClient.GetFromJsonAsync<Movie>(
                $"3/search/movie?query={Uri.EscapeDataString(query)}&api_key={ApiKey}&language=en-US");
